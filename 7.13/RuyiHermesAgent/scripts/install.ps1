        ...
        if ($repoValid) {
            ...
            try {
                ...
                if ($LASTEXITCODE -ne 0) {
                    Write-Warn "git fetch origin $Branch failed (exit $LASTEXITCODE); remote is unreachable."
                    Write-Warn "Using the existing local installation as-is."
                    # Leave the managed repo untouched and skip the checkout/pull
                    # so bootstrap can continue with the code that is already on disk.
                    if ($autostashRef) {
                        Write-Warn "Local changes were stashed before the failed fetch."
